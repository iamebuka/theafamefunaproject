...
const { aggregate, notd, httpError } = require("../utils");
...
router.post("/contribute", function (req, res) {
  ...
  suggestion.save(function (err) {
    ...
    req.flash(
      "success_messages",
      `The entry "${req.body.name}" has been received.<br/>
  ...
  afamefunaModel
    ...
    .then((data) => {
      if (data.length) {
        ...
        next(new httpError(404, "Ops! page doesnt exist"));
      }
    });
});
...
router.get("/entries/:name", function (req, res, next) {
  ...
  afamefunaModel
    .findOneAndUpdate(
      { name: { $regex: "^" + nameParam + "$", $options: "ix" } },
      { $inc: { lookup_count: 1 } }
    )
    .exec()
    .catch(function (err) {
